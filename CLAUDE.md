...
- Gap analysis (2026-05-08, post-hygiene-sweep): **0 Critical, 0 Important, 4 Minor.** The 4 Minor are 4 Unverified ASMs (3 Medium-risk: interpretation, newsletter, geocoding; 1 Low-risk: de-en-covers-target-audience), all with explicit Verification Plans dated 2026-05-08. Each plan names the trigger that flips the ASM to Verified. Spec → Design gate fully clean.
...
- **Completeness assessment (2026-05-08, post-hygiene-sweep): 0 Critical, 0 Important, 4 Minor.** The 4 Minor are 4 Unverified ASMs (3 Medium + 1 Low) all with Verification Plans dated 2026-05-08; resolution paths captured in tasks.md (vendor decisions for 3 of them, post-launch analytics for the 4th). Design → Code gate fully clean (0 Critical + fresh).
...
- 2026-05-08: SDLC hygiene sweep — closed 4 procedural follow-ups left after the FuFirE alignment. (1) Re-approved `CON-fufire-chart-endpoint` and `REQ-F-fufire-chart-mapping` after `STK-founder` review of the new `/v1/fusion` wording — restores 23 Approved REQs / 7 Active CONs (commit `e09ca34`). (2) Added Verification Plans to 4 Unverified ASMs (3 vendor + 1 language) so the path Unverified → Verified is concrete and trackable; flagged prospective DECs explicitly to avoid dead-link confusion (commits `78b0e21` + `7a3f150`). (3) Refreshed gap analysis + completeness assessment — both 0 Critical, 0 Important, 4 Minor; phase gates fully clean (commit `0fa8e31`). (4) Tracked Fly.io-vs-Railway URL ambiguity as `TASK-resolve-fufire-url-ambiguity`. Closes `TASK-reapprove-fufire-artefacts` and `TASK-refresh-spec-design-assessments`. See [`docs/plans/2026-05-08-sdlc-hygiene-sweep.md`](docs/plans/2026-05-08-sdlc-hygiene-sweep.md).
