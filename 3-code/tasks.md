...
|----|------|----------|--------|-----|--------------|---------|-------|
| TASK-decide-fufire-deployment | Verify FuFirE engine readiness (URL + API key obtainable); record `DEC-fufire-baseline` with deployment rollout pattern | P2 | Done | [REQ-F-fufire-chart-mapping](../1-spec/requirements/REQ-F-fufire-chart-mapping.md) | - | 2026-05-08 | Done 2026-05-08: superseded by DEC-fufire-baseline (alignment with deployed BAFE engine, not a build/buy choice). |
| TASK-decide-geocoding-vendor | BAFE accepts birth.lat / birth.lon directly. Determine whether the Waitinglist frontend still needs a geocoding vendor for place-label-to-coordinates UX. If yes, compare Mapbox/OpenCage/Google. If no, record DEC-no-geocoding-vendor. | P2 | Todo | - | - | 2026-05-08 | Briefing rewritten 2026-05-08 — geocoding may be redundant if frontend captures lat/lon via browser geolocation or numeric input. |
| TASK-decide-interpretation-vendor | Determine whether BAFE's /v1/experience/daily (returns summary, themes, caution, opportunity natively) makes a separate interpretation vendor unnecessary. If yes, record DEC-no-separate-interpretation-vendor. If no, compare Gemini direct vs proxy. | P2 | Todo | - | - | 2026-05-08 | Briefing rewritten 2026-05-08 after BAFE API snapshot revealed native interpretation surface. |
...
| TASK-update-assumption-statuses | Link the 4 `ASM-*` files to their newly-recorded `DEC-*` records; keep status Unverified until live test in Phase 4 | P2 | Todo | - | TASK-decide-fufire-deployment, TASK-decide-geocoding-vendor, TASK-decide-interpretation-vendor, TASK-decide-newsletter-vendor | 2026-05-07 | Bidirectional traceability — partially done 2026-05-08 for ASM-fufire-api-available (linked to DEC-fufire-baseline + status Verified). 3 ASMs remain. |
| TASK-reapprove-fufire-artefacts | Re-approve `CON-fufire-chart-endpoint` (Draft) and `REQ-F-fufire-chart-mapping` (Draft) via `/SDLC-elicit` after `STK-founder` reviews the new `/v1/fusion` text | P2 | Done | [REQ-F-fufire-chart-mapping](../1-spec/requirements/REQ-F-fufire-chart-mapping.md) | TASK-decide-fufire-deployment | 2026-05-08 | Done 2026-05-08: re-approved by STK-founder, Status flipped back to Approved for both CON-fufire-chart-endpoint and REQ-F-fufire-chart-mapping. Counts in CLAUDE.md restored to 23 Approved REQs / 7 Active CONs. |
| TASK-refresh-spec-design-assessments | Re-run gap analysis (Spec) + completeness assessment (Design) to refresh stale numbers in `CLAUDE.md` after the 2026-05-08 ASM-Verified + CON-Draft + REQ-Draft transitions | P3 | Done | - | TASK-reapprove-fufire-artefacts | 2026-05-08 | Done 2026-05-08: post-hygiene-sweep refresh — 0 Critical, 0 Important, 4 Minor for both Spec gap analysis and Design completeness assessment. Both phase gates now formally clean. |
